...
serve(async (req) => {
  ...
  try {
    ...
    try {
      ...
      console.log(`📊 Fetched ${markets.length} established coins for ${PLAN.toUpperCase()} plan`);

      const predictions = [];

      // 5. Gerar exatamente TARGET_COUNT palpites de risco médio
...
📊 ${PLAN.toUpperCase()} Plan Generation Summary:
  ...
  - Expires in: ${EXPIRES_IN_HOURS} hour
      `);
